...
pega.util.Get = function() {
    ...
    var _node = function(type, attr, win) {
        ...
        for (var i in attr) {
            if (attr[i] && pega.lang.hasOwnProperty(attr, i)) {
    ...
    var _next = function(id, loaded) {
        ...
        if (q.url.length === 0) {
            ...
            if (q.type === "script" && ua.webkit && ua.webkit < 420 && 
                ...
                extra.innerHTML='pega.util.Get._finalize("' + id + '");';
    ...
    var _queue = function(type, url, opts) {
        ...
        if (qidx % pega.util.Get.PURGE_THRESH === 0) {
            _autoPurge();
        }
        ...
        // webkit prior to 3.x is problemmatic
            ...
            if (type === "script") {
                ...
                // property that the utility can poll for.
                    ...
                    var q = queues[id];
                    if (q.varName) {
                        var freq=pega.util.Get.POLL_FREQ;
                        q.maxattempts = pega.util.Get.TIMEOUT/freq;
                        q.attempts = 0;
                        ...
                        lang.later(pega.util.Get.POLL_FREQ, null, f, [id, url]);
                    }
                }
            } 
    ...
    return {
         ...
         * <pre>
         * // assumes pega, dom, and event are already on the page
         * &nbsp;&nbsp;pega.util.Get.script(
         * &nbsp;&nbsp;["http://yui.pegaapis.com/2.3.1/build/dragdrop/dragdrop-min.js",
         * &nbsp;&nbsp;&nbsp;"http://yui.pegaapis.com/2.3.1/build/animation/animation-min.js"], &#123;
         * &nbsp;&nbsp;&nbsp;&nbsp;onSuccess: function(o) &#123;
         * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;new pega.util.DDProxy("dd1"); // also new o.reference("dd1"); would work
         * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;this.log("won't cause error because pega is the scope");
         ...
         * </dl>
         * <pre>
         *      pega.util.Get.css("http://yui.pegaapis.com/2.3.1/build/menu/assets/skins/sam/menu.css");
         * </pre>
         * <pre>
         *      pega.util.Get.css(["http://yui.pegaapis.com/2.3.1/build/menu/assets/skins/sam/menu.css",
         * </pre>
...
pega.register("get", pega.util.Get, {version: "2.5.1", build: "984"});
/**
 ...
 * and pega-dom-event.js), and will automatically use these when
 ...
 * @class pega.util.YUILoader
 * @todo
 *      version management, automatic sandboxing
 */
(function() {

    var Y=pega, util=Y.util, lang=Y.lang, env=Y.env,
        PROV = "_provides", SUPER = "_supersedes",
        REQ = "expanded", AFTER = "_after";
 
    var YUI = {

        dupsAllowed: {'pega': true, 'get': true},

        /*
         * The library metadata for the current release  The is the default
         * value for pega.util.YUILoader.moduleInfo
         * @property YUIInfo
         * @static
         */
        info: {
    'base': 'http://yui.pegaapis.com/2.5.1/build/',
    ...
    'moduleInfo': {
        ...
        'utilities': {
            ...
            'supersedes': ['pega', 'event', 'dragdrop', 'animation', 'dom', 'connection', 'element', 'pega-dom-event', 'get', 'yuiloader', 'yuiloader-dom-event'],
            'rollup': 8
        },

        'pega': {
            'type': 'js',
            'path': 'pega/pega-min.js'
        },

        'pega-dom-event': {
            'type': 'js',
            'path': 'pega-dom-event/pega-dom-event.js',
            'supersedes': ['pega', 'event', 'dom'],
            'rollup': 3
        },
        ...
        'yuiloader-dom-event': {
            ...
            'supersedes': ['pega', 'dom', 'event', 'get', 'yuiloader', 'pega-dom-event'],
            'rollup': 5
        },
 ...
 , 
    ...
    pega.util.YUILoader = function(o) {
        ...
        /**
         * Use the pega environment listener to detect script load.  This
         * is only switched on for Safari 2.x and below.
         * @property _usepegaListener
         * @private
         */
        this._usepegaListener = false;
         ...
         * @property base
         * @type string
         * @default http://yui.pegaapis.com/[YUI VERSION]/build/
         */
         ...
         * Composed of what pega reports to be loaded combined
         ...
         *      // http://yui.pegaapis.com/2.3.0/build/assets/skins/sam/
         *      base: 'assets/skins/',
         *
        ...
        env.listeners.push(function(m) {
            if (self._usepegaListener) {
                //Y.log("pega listener: " + m.name);
...
// Y.log(name + " provides " + lang.dump(m[PROV], 0));
         ...
         * provided a reference to the sandboxed pega object.  This only
        ...
        sandbox: function(o, type) {
            if (o) {
                // pega.log("sandbox: " + lang.dump(o, 1) + ", " + type);
            } else {
                // pega.log("sandbox: " + this.toString() + ", " + type);
            }
...
throw new Error("You must supply an onSuccess handler for your sandbox");
            ...
            for (i=0; i<l; i=i+1) {
                ...
                var xhrData = {
                    ...
                    success: function(o) {
                        ...
                        // pega.log("received: " + o.responseText.substr(0, 100) + ", " + idx);
         ...
         * pega.register to determine when scripts are fully loaded
         * @method loadNext
        ...
        loadNext: function(mname) {
            ...
            if (mname) {
                ...
                // pega.log("loadNext executing, just loaded " + mname);
            ...
            for (i=0; i<len; i=i+1) {
                ...
                if (s[i] in this.inserted) {
                    // pega.log(s[i] + " alread loaded ");
                    continue;
                }

                // Because rollups will cause multiple load notifications
                // from pega, loadNext may be called multiple times for
                // the same module when loading a rollup.  We can safely
                // skip the subsequent requests
                if (s[i] === this._loading) {
                    // pega.log("still loading " + s[i] + ", waiting");
                    return;
                }
                ...
                if (!this.loadType || this.loadType === m.type) {
                    this._loading = s[i];
                    //pega.log("attempting to load " + s[i] + ", " + this.base);
                    ...
                    if (env.ua.webkit && env.ua.webkit < 420 && m.type === "js" && 
                          ...
                          //pega.log("using pega env " + s[i] + ", " + m.varName);
                        c = null;
                        this._usepegaListener = true;
                    }
